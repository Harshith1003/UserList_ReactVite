...
const UserList = () => {
  const [users, setUsers] = useState(celebrities);
  const [activeAccordion, setActiveAccordion] = useState(null);
  const [editMode, setEditMode] = useState(null);
  const [searchQuery, setSearchQuery] = useState("");
  const [editedUser, setEditedUser] = useState(null); // New state to store edited user data
  ...
  const handleCancelClick = (userId) => {
    ...
    if (editedUser) {
      setUsers((prevUsers) =>
        prevUsers.map((user) => (user.id === userId ? { ...editedUser } : user))
      );
    }
  };
